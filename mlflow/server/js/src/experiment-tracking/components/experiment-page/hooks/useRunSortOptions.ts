...
export const useRunSortOptions = (
  categorizedUncheckedKeys: ExperimentCategorizedUncheckedKeys,
  ...
  useMemo(() => {
    ...
    const attributesSortBy = Object.keys(ATTRIBUTE_COLUMN_SORT_LABEL).reduce<any[]>(
      (options, sortLabelKey) => {
        ...
        const attributeColumnName =
          COLUMN_TYPES.ATTRIBUTES as keyof ExperimentCategorizedUncheckedKeys;

        if (!categorizedUncheckedKeys[attributeColumnName].includes(sortLabel)) {
    ...
    return sortOptions;
  }, [categorizedUncheckedKeys, filteredMetricKeys, filteredParamKeys]);
